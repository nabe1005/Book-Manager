...
@login_required
def index():
    series_n = Book.query.filter_by(user_id=current_user.id).order_by(
        Book.series_id.desc()).first().series_id
    series_n += 1
    res = dict()
    ...
    for i in range(1, series_n):
        books = Book.query.filter_by(
            series_id=i, user_id=current_user.id).order_by(Book.vol).all()
        display_vol = ''
        ...
        book = {i: {'name': book.book_name,
                    'vol': display_vol, 'categories': categories}}
        res.update(book)
...
def register_post():
    ...
    if not series:
        if Book.query.filter_by(book_name=book_name, user_id=current_user.id).first():
            flash('既に追加されています')
            return redirect(url_for('books.register'))
        new_book = Book(series_id=series_id, book_name=book_name,
                        vol=1, user_id=current_user.id)
        ...
        for i in range(int(vol)):
            ...
            if request.form.get('vol' + str(i + 1)):
                new_book = Book(
                    series_id=series_id, book_name=book_name, vol=i + 1, user_id=current_user.id)
                new_books.append(new_book)
...
@login_required
def edit(series_id):
    edit_books = Book.query.filter_by(
        series_id=series_id, user_id=current_user.id).order_by(Book.vol).all()
...
def edit_post(series_id):
    ...
    if len(Book.query.filter_by(series_id=series_id, user_id=current_user.id).all()) > int(vol):
        i_length = len(Book.query.filter_by(
            series_id=series_id, user_id=current_user.id).all()) + 1
    else:
        i_length = int(vol) + 1
        ...
        if request.form.get('vol' + str(i)) and i <= int(vol):
            if Book.query.filter_by(series_id=series_id, vol=i, user_id=current_user.id).first():
                add_book = Book.query.filter_by(
                    series_id=series_id, vol=i, user_id=current_user.id).first()
                add_book.book_name = book_name
                add_books.append(add_book)
            else:
                add_books.append(
                    Book(book_name=book_name, series_id=series_id, vol=i, user_id=current_user.id))
        else:
            if Book.query.filter_by(series_id=series_id, vol=i, user_id=current_user.id).first():
                Book.query.filter_by(series_id=series_id,
                                     vol=i, user_id=current_user.id).delete()
...
def delete(series_id):
    delete_books = Book.query.filter_by(
        series_id=series_id, user_id=current_user.id).all()
